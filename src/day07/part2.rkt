...
(define beams (make-vector cols))
(define answer 0)

(vector-set! beams (string-first-char-occurrence (list-ref lines 0) #\S) 1)
...
(for ([row (range 1 rows)])
  (for ([col (range cols)])
    ...
    (when (equal? (string-ref (list-ref lines row) col) #\^)
      (vector-set! beams (sub1 col) (+ (vector-ref beams (sub1 col)) (vector-ref beams col)))
      (vector-set! beams (add1 col) (+ (vector-ref beams (add1 col)) (vector-ref beams col)))
      (vector-set! beams col 0)
    )
  )
)

(printf "Answer: ~v~n" (apply + (vector->list beams)))
